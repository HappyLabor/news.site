...
document.addEventListener("DOMContentLoaded", function () {
  // 1. 在这里定义你的新闻数据 (手动更新此处即可)
  const newsData = [
    {
      date: "2026-03-15",
      content: "祝贺课题组李四同学在 Nature Neuroscience 发表最新研究成果！",
      highlight: true, // true 表示重要新闻（橙色点），false 表示普通（蓝色点）
    },
    {
      date: "2026-02-10",
      content: "实验室获得国家自然科学基金重点项目资助。",
      highlight: false,
    },
    {
      date: "2026-01-05",
      content: "欢迎 2026 级博士研究生加入 HappyLab 大家庭。",
      highlight: false,
    },
  ];
  ...
  if (newsContainer) {
    ...
    if (typeof observeElements === "function") {
      observeElements(newsItems);
    }
  }

  // ... (保留原本的 Publications 数据和逻辑) ...
  // ... (保留原本的 Modal 和 Form 逻辑) ...
});
